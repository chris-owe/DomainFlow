...
import type { Domain, Task, Note } from '../types'
...
export default function DomainView() {
  ...
  const { tasks, loading: tasksLoading, createTask, updateTask, deleteTask, toggleTask } = useTasks(id)
  ...
  useEffect(() => {
    ...
    if (found) { setDomain(found); return }
    supabase.from('domains').select('*').eq('id', id).single().then(({ data }) => {
      if (data) setDomain(data)
    })
  }, [id, domains])

  if (!domain) {
    return <div style={{ color: '#64748B', padding: '40px 0', textAlign: 'center' }}>领域不存在</div>
  }

  const activeTasks = tasks.filter(t => t.status !== 'done')
  const doneTasks = tasks.filter(t => t.status === 'done')
  const domainNames = { [domain.id]: domain.name }
  const domainColors = { [domain.id]: domain.color }
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: '16px' }}>
        ...
        {domain.description && (
          <p style={{ fontSize: '13px', color: '#64748B', marginTop: '4px' }}>{domain.description}</p>
        )}
      </div>
      ...
      {tab === 'all' && (
        <>
          {tasksLoading || notesLoading ? (
            ...
            <div style={{ color: '#64748B', padding: '40px 0', textAlign: 'center' }}>该领域暂无内容</div>
          ) : (
        ...
        <>
          {tasksLoading ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              {doneTasks.length > 0 && (
                <div>
                  <div style={{ fontSize: '13px', color: '#475569', marginBottom: '8px', marginTop: '8px' }}>已完成（{doneTasks.length}）</div>
                  <TaskList tasks={doneTasks} domainNames={domainNames} domainColors={domainColors} onToggle={toggleTask} onEdit={setEditingTask} />
                </div>
              )}
            </div>
      ...
      {/* Task dialogs */}
      {showCreateTask && (
        <TaskDialog mode="create" domains={[domain]} onClose={() => setShowCreateTask(false)}
          onConfirm={async (data: TaskInput) => { const ok = await createTask(data); if (ok) setShowCreateTask(false); return ok }} />
      )}
      {editingTask && (
        <TaskDialog mode="edit" domains={[domain]} task={editingTask} onClose={() => setEditingTask(null)}
      ...
      {/* Note dialogs */}
      {showCreateNote && (
        <NoteDialog mode="create" domains={[domain]} onClose={() => setShowCreateNote(false)}
          onConfirm={async (data: NoteInput) => { const ok = await createNote(data); if (ok) setShowCreateNote(false); return ok }} />
      )}
      {editingNote && (
        <NoteDialog mode="edit" domains={[domain]} note={editingNote} onClose={() => setEditingNote(null)}
...
const createBtnStyle: React.CSSProperties = {
  ...
  background: '#06B6D4', color: '#fff', fontSize: '13px', cursor: 'pointer', marginBottom: '8px', fontWeight: 500,
}
